...
@interface WBView()<UIGestureRecognizerDelegate>
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    WBPoint normalDirection;
    CGPoint last;
...
- (void)setItems:(NSArray *)items
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        WBPoint point = WBPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(WBPoint)];
    ...
    normalDirection = WBPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(WBPoint)direction andAngle:(CGFloat)angle
{
    ...
    WBPoint rPoint = WBPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(WBPoint)];
...
- (void)setTagOfPoint: (WBPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        WBPoint direction = WBPointMake(last.y - current.y, current.x - last.x, 0);
